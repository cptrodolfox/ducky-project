...
-- | Looks projects associated to a Grant.
projectsOnGrant :: GrantId -> Query HasRelDb [ProjectId]
projectsOnGrant grant = (foldr (\(ProjectHasGrant pro g) l ->
                                  if g == grant
                                  then pro : l
                                  else l) [])
                        . allHasRels <$> ask

-- | Looks for the Students working on a Project.
studentsOnProject :: ProjectId -> Query WorksOnDb [DNI]
studentsOnProject pro = (foldr (\(StudentWorksOn dni p) l ->
                                  if p == pro
                                  then dni : l
                                  else l) [])
                        . allWorksOn <$> ask

-- | Looks for the Employees working on a Project.
employeesOnProject :: ProjectId -> Query WorksOnDb [DNI]
employeesOnProject pro = (foldr (\(EmployeeWorksOn dni p) l ->
                                   if p == pro
                                   then dni : l
                                   else l) [])
                         . allWorksOn <$> ask

-- | Looks for projects where a student works.
projectsOfStudent :: DNI -> Query WorksOnDb [ProjectId]
projectsOfStudent dni = (foldr (\(StudentWorksOn d pro) l ->
                                  if d == dni
                                  then pro : l
                                  else l) [])
                        . allWorksOn <$> ask

-- | Looks for the projects where an employee works.
projectsOfEmployee :: DNI -> Query WorksOnDb [ProjectId]
projectsOfEmployee dni = (foldr (\(EmployeeWorksOn d pro) l ->
                                   if d == dni then pro : l else l) [])
                         . allWorksOn <$> ask
